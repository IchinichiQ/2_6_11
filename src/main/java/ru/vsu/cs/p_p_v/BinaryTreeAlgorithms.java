...
public class BinaryTreeAlgorithms {
    ...
    public interface Visitor<T> {
         ...
         */
        void visit(BinaryTree.TreeNode<T> node);
    }
    ...
    public static <T> void preOrderVisit(BinaryTree.TreeNode<T> treeNode, Visitor<T> visitor) {
        ...
        class Inner {
            void preOrderVisit(BinaryTree.TreeNode<T> node, Visitor<T> visitor) {
                if (node == null) {
                    return;
                }
                visitor.visit(node);
                preOrderVisit(node.getLeft(), visitor);
                preOrderVisit(node.getRight(), visitor);
            }
        }
        // класс приходится создавать, т.к. статические методы в таких класс не поддерживаются
        new Inner().preOrderVisit(treeNode, visitor);
    }
    ...
    public static <T> void inOrderVisit(BinaryTree.TreeNode<T> treeNode, Visitor<T> visitor) {
        // данный класс нужен только для того, чтобы "спрятать" его метод (c 3-мя параметрами)
        class Inner {
            void inOrderVisit(BinaryTree.TreeNode<T> node, Visitor<T> visitor) {
                if (node == null) {
                    return;
                }
                inOrderVisit(node.getLeft(), visitor);
                visitor.visit(node);
                inOrderVisit(node.getRight(), visitor);
            }
        }
        // класс приходится создавать, т.к. статические методы в таких класс не поддерживаются
        new Inner().inOrderVisit(treeNode, visitor);
    }
    ...
    public static <T> void postOrderVisit(BinaryTree.TreeNode<T> treeNode, Visitor<T> visitor) {
        // данный класс нужен только для того, чтобы "спрятать" его метод (c 3-мя параметрами)
        class Inner {
            void postOrderVisit(BinaryTree.TreeNode<T> node, Visitor<T> visitor) {
                if (node == null) {
                    return;
                }
                postOrderVisit(node.getLeft(), visitor);
                postOrderVisit(node.getRight(), visitor);
                visitor.visit(node);
            }
        }
        // класс приходится создавать, т.к. статические методы в таких класс не поддерживаются
        new Inner().postOrderVisit(treeNode, visitor);
    }
     ...
     */
    public static <T> String toGraphvizStr(BinaryTree.TreeNode<T> treeNode) {
